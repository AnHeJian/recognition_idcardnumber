...
blkBackground = np.zeros((28,28),np.uint8)
...
def Denoise(img):
    th = 100
    temImg = np.zeros((img.shape[0], img.shape[1]),np.uint8)
...
def Denoise2(img):
    ...
    for x in range(img.shape[0]):
        for y in range(img.shape[1]):
            hist[int(img[x,y]/step)] = hist[int(img[x,y]/step)]+1
    bg = 1
    for i in range(histSize-1, -1, -1):
        if hist[i] > threshold:
            bg = i
            if i > 2:
                bg = bg-1
...
def printImg(img):  
    ...
    for x in range(img.shape[0]):
        for y in range(img.shape[1]):
            hist[int(img[x,y]/step)] = hist[int(img[x,y]/step)]+1
    print hist        
#iMaxSharp = Sharp(temImg)  #
#iAddSharp = Sharp(temImg,1) #
#iRMaxSharp = Sharp(temImg,0,1) #
#iRAddSharp = Sharp(temImg,1,1) #        
def Sharp(image,flag1=0,flag2=0):
    w = image.shape[0]
    h = image.shape[1]
    iSharp = np.zeros((w,h),np.uint8)
    for i in range(w-1):
        for j in range(h-1):
            if flag2 == 0:
...
def ProcessIDNumber(img_ID):
    ...
    for i in range(startIDNumIdx, len(valley)-1):
        ...
        if temImg is not None:
            ...
            temImg = FixedShape(temImg)
            temImg = Denoise(temImg)            
            temImg = Denoise2(temImg)            
...
def RecogIDNumber(imgList):
    ...
    test  = mx.io.MNISTIter(
        ...
        batch_size = 1,
        shuffle    = 0,)
    #model = mx.model.FeedForward.load('model_card', 20)
    model = mx.model.FeedForward.load('model_integer_mlp', 20)
    ...
    #print int(IDNumber[6:10]),u'年', int(IDNumber[10:12]),u'月', int(IDNumber[12:14]),u'日'
    
def RecogSex(imgList):
    val_img_buf = ''
    val_lab_buf = ''
    val_img_num = 0
    rows = 28
...
def RecogNation(imgList):
    ...
    [probs, data, label] = model.predict(X = test, return_data=True)
    chlist = ReadTextFile('transChineseTable.txt')
    for i in range(len(probs)):
        idnum = np.argmax(probs[i])
        print chlist[idnum],
    print
def ReadTextFile(fileName):
    ...
    for i in range(len(chlist)):
        chlist[i]=chlist[i].strip('\n')
        #print chlist[i],
    #print
    return chlist
def RecogName(imgList):
    val_img_buf = ''
    val_lab_buf = ''
    val_img_num = 0
    rows = 28
...
if __name__ == '__main__':    
    for id in xrange(0,7):    
        ...
        print 'sex length', len(imgSexList)
        RecogSex(imgSexList[0])
        RecogNation(imgSexList[1: ])
        ############################################
        #process birth
        ############################################
